...
class EqualityMatrix {
	...
	get w() {
		return this.get(Direction.W);
	}
	set w(flag) {
		this.set(Direction.W, flag);
	}

	get nw() {
		return this.get(Direction.NW);
	}
	set nw(flag) {
		this.set(Direction.NW, flag);
	}

	get n() {
		return this.get(Direction.N);
	}
	set n(flag) {
		this.set(Direction.N, flag);
	}

	get ne() {
		return this.get(Direction.NE);
	}
	set ne(flag) {
		this.set(Direction.NE, flag);
	}

	get e() {
		return this.get(Direction.E);
	}
	set e(flag) {
		this.set(Direction.E, flag);
	}

	get se() {
		return this.get(Direction.SE);
	}
	set se(flag) {
		this.set(Direction.SE, flag);
	}

	get s() {
		return this.get(Direction.S);
	}
	set s(flag) {
		this.set(Direction.S, flag);
	}

	get sw() {
		return this.get(Direction.SW);
	}
	set sw(flag) {
		this.set(Direction.SW, flag);
	}

	toBraille() {
		return (
			getBrailleColumn(this.nw, this.w, this.sw) +
...
function getBrailleColumn(top, middle, bottom) {
	const codepoint =
		0x2800 | (top ? 1 : 0) | (middle ? 2 : 0) | (bottom ? 4 : 0);
...
function generatePixelUpscaleCode(
	matrix,
	flags,
	pixelRecords,
	{ generateBreak = true } = {}
	...
	for (const pixelRecord of pixelRecords) {
		...
		if (useSourceColor) {
			...
			const sourceDirections = pixelRecord.sourceDirections.filter(
				(d) => !matrix.get(d)
			);
			const value =
				sourceDirections
					...
					.map((d) => {
						...
						const otherValueCode = `src.get(x${formatOffset(
							vector[0]
						)}, y${formatOffset(vector[1])})`;
						return `!linePixels.get(x${formatOffset(
							vector[0]
						)}, y${formatOffset(
							vector[1]
						)}) ? ${otherValueCode} : `;
...
function generateScalePixelFunction(width, height, pixelRecords) {
	...
	const switchBlock = generateSwitchBlock("matrix", (matrix) => {
		...
		switch (pixelType) {
			case PixelType.LINE:
				return generatePixelUpscaleCode(
					matrix,
					getLineUpscaleFlags(matrix),
					pixelRecords
				);
			case PixelType.FILL:
				return generatePixelUpscaleCode(
					matrix,
					getFillUpscaleFlags(matrix),
					pixelRecords
				);
			case PixelType.INDETERMINATE:
				const lineUpscaleCode = generatePixelUpscaleCode(
					matrix,
					getLineUpscaleFlags(matrix),
					pixelRecords,
					{ generateBreak: false }
				);
				const fillUpscaleCode = generatePixelUpscaleCode(
					matrix,
					getFillUpscaleFlags(matrix),
					pixelRecords,
					{ generateBreak: false }
	...
	const pixelRecords = [
		{
			param: "topLeft",
			flagIndexes: [0],
			sourceDirections: [Direction.N, Direction.W],
		},
		{
			param: "topRight",
			flagIndexes: [1],
			sourceDirections: [Direction.N, Direction.E],
		},
		{
			param: "bottomLeft",
			flagIndexes: [2],
			sourceDirections: [Direction.S, Direction.W],
		},
		{
			param: "bottomRight",
			flagIndexes: [3],
			sourceDirections: [Direction.S, Direction.E],
		},
	];
	...
	const pixelRecords = [
		{
			param: "left",
			flagIndexes: [0, 2],
			sourceDirections: [Direction.N, Direction.W, Direction.S],
		},
		{
			param: "right",
			flagIndexes: [1, 3],
			sourceDirections: [Direction.N, Direction.E, Direction.S],
		},
	];
	...
	const pixelRecords = [
		{
			param: "top",
			flagIndexes: [0, 1],
			sourceDirections: [Direction.N, Direction.W, Direction.E],
		},
		{
			param: "bottom",
			flagIndexes: [2, 3],
			sourceDirections: [Direction.S, Direction.W, Direction.E],
		},
	];
...
const generators = [
	generateScalePixelTo2x2,
	generateScalePixelTo2x1,
	generateScalePixelTo1x2,
];
