...
// analog joystick axis id (for internal use) - Should match the logic in NovelVMEvents.java
enum {
...
//			LOGD("JE_TAP - VALID: ev0: mx %d my %d t: %d | ev1: mx %d my %d t: %d | ev2: mx %d my %d t: %d", ev0.mouse.x, ev0.mouse.y, ev0.type, ev1.mouse.x, ev1.mouse.y, ev1.type, ev2.mouse.x, ev2.mouse.y, ev2.type);
	...
	case JE_DOUBLE_TAP:
		...
		//       Before the ACTION_DOWN event, we also have ALREADY pushed a JE_DOWN event (via NovelVMEvents's onDown())
		//       and then a JE_TAP event (action UP) via NovelVMEvents's onSingleTapUp().
		//       Before the ACTION_UP event, we also have ALREADY pushed a JE_DOWN event (via NovelVMEvents's onDown()).
