...
mkdir -p novelvm/bin-$1

if [ \! -f novelvm/bin-$1/config.mk ] ; then
	$ANDROIDSDL/project/jni/application/setEnvironment-$1.sh sh -c "cd novelvm/bin-$1 && env LIBS='-lflac -lvorbis -logg -lmad -lz -lgcc -ltheora -lpng -lfreetype -lfaad -lgnustl_static' ../configure --host=androidsdl-$1 --enable-optimizations --enable-release --enable-zlib --enable-vorbis --enable-mad --enable-flac --enable-png --enable-theoradec --disable-sdlnet --disable-libcurl --disable-cloud --enable-vkeybd --enable-mt32emu --disable-readline --disable-nasm --disable-timidity --disable-fluidsynth --datadir=. "
fi
$ANDROIDSDL/project/jni/application/setEnvironment-$1.sh make -j4 -C novelvm/bin-$1
make -C novelvm/bin-$1 androidsdl
mv -f novelvm/bin-$1/novelvm*.z* AndroidData
cp -f novelvm/bin-$1/novelvm libapplication-$1.so
