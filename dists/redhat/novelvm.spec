...
#------------------------------------------------------------------------------
%description
NovelVM is an interpreter that will play many graphic adventure games,
...
games) and many more. See https://www.novelvm.org for a full compatibility list.

#------------------------------------------------------------------------------
#   install scripts
#------------------------------------------------------------------------------
%prep
%setup -q -a 1 -a 2 -n novelvm-%{version}
mkdir tmp

%build
(cd faad2-2.8.6; ./configure --enable-static --disable-shared --prefix=%{_builddir}/novelvm-%{version}/tmp; make %{_smp_mflags} -C libfaad all install)
(cd libmpeg2-0.5.1; ./configure --enable-static --disable-shared --prefix=%{_builddir}/novelvm-%{version}/tmp; make -C libmpeg2 all install; make -C include all install)
./configure --with-faad-prefix=%{_builddir}/novelvm-%{version}/tmp --with-mpeg2-prefix=%{_builddir}/novelvm-%{version}/tmp --prefix=%{_prefix} --enable-release
make %{_smp_mflags}
...
%{_datadir}/metainfo/novelvm.appdata.xml
%{_datadir}/novelvm/*
...
* Thu Jul 15 2004 (0.6.0-2)
  ...
  - used some elements of the .spec file for novelvm from http://livna.org/
